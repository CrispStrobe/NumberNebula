...
class SriService with ChangeNotifier {
  ...
  // criterion mirrors isProblemMastered above.

  /// Returns 1..5 for the given problem data. 1 = Neu, 5 = Gemeistert.
  int getBoxFor(SriProblemData d) {
  ...
  List<SriProblemData> getItemsInBox(int box) {
  ...
  Future<void> moveItemToBox(String problemId, int targetBox) async {
    ...
    switch (targetBox) {
      ...
      case 4:
        ...
        // Stay below mastery: cap EF just under threshold or bump failureCount.
    ...
    _log('Moved "$problemId" to box $targetBox');
    notifyListeners();
    await saveSriData();
  }
}
