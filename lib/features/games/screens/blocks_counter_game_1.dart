...
class _BlockCounterGameState extends State<BlockCounterGame> with TickerProviderStateMixin {
  ...
  void _generatePuzzle() async {
    ...
    setState(() {
      _isGenerating = true;
      _cubeKey = UniqueKey();
      // Reset controllers but don't clear currentPuzzle yet to avoid UI issues
      _successController.reset();
      debugPrint("🧊 State reset: _isGenerating = true, controllers reset");
    });
    ...
    try {
      ...
      debugPrint("🧊 Puzzle computation completed: ${puzzle.blockStructure.blocks.length} blocks, correct answer: ${puzzle.correctAnswer}");
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          debugPrint("🧊 Creating 3D scene object...");
          _createSceneObject();
          _isGenerating = false;
          debugPrint("🧊 State updated: _isGenerating = false, scene created");
        });
        debugPrint("🧊 Puzzle generation completed successfully");
      } else {
      ...
      if (mounted) {
        ...
        debugPrint("🧊 Error state: _isGenerating = false");
      }
    }
  }
  
  void _createSceneObject() {
    debugPrint("🧊 _createSceneObject() - Starting 3D scene creation");
    
    if (currentPuzzle == null) {
      debugPrint("🧊 ❌ Cannot create scene - currentPuzzle is null!");
      return;
    }
                ...
                builder: (context, child) {
                  return cube.Cube(
                    ...
                    onSceneCreated: (cube.Scene scene) {
                      // Apply rotation to the scene object instead of camera
