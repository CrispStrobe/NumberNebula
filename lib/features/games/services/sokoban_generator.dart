...
//
// lib/features/games/services/sokoban_generator.dart
//
// Difficulty-parametrized Sokoban level generator (pure Dart, dart:math only —
// safe to import anywhere in the app). This is the CORE algorithm; the CLI /
// HTML front-end lives in tool/sokoban_generator.dart, which imports this file.
//
...
//
// Cells are encoded as ints `r * 256 + c` (stride 256, coords < 256), which
// makes floor/box/goal sets plain Set<int> and directions plain int deltas:
// UP=-256, DOWN=256, LEFT=-1, RIGHT=1.
//
// The [sokobanToStarLoaderEntry] converter at the bottom turns a [Level] into
// the roomStructure/roomState int-grid schema the Star Loader game consumes.
...
List<_Candidate> backwardScramble(
  ...
  for (final entry in pool) {
    ...
    seenBoxes.add(bk);
    out.add(
        _Candidate(boxes, entry[4] as int, entry[1] as int, entry[2] as int));
...
///
/// [onAttempt], when given, is called once per solved candidate with a
/// human-readable metrics line (used by the CLI's verbose flag).
Level generate(
  int difficulty, {
  int? seed,
  int maxAttempts = 250,
  ...
  for (var attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    for (final cnd in cands.take(3)) {
      ...
      if (onAttempt != null) {
        onAttempt('  attempt $attempt: pushes=${m.pushes} '
...
// ------------------------------------------------- Star Loader integration
// Tile encoding shared with StarLoaderLevelManager / LevelGenerator:
//   structure: WALL=0, FLOOR=1, TARGET=2
//   state:     WALL=0, FLOOR=1, TARGET=2, BOX=4, PLAYER=5  (mirrors structure
//              for non-entity cells). A box on a target is encoded as BOX(4)
//              with structure TARGET(2) — the game renders that as 'X'.
const int slWall = 0;
const int slFloor = 1;
const int slTarget = 2;
const int slBox = 4;
const int slPlayer = 5;

/// A generated Star Loader level in the manager's int-grid schema.
class StarLoaderEntry {
  const StarLoaderEntry({
  ...
  /// Push-optimal solution length (matches the game's own solver).
  final int optimalMoves;

  /// Grid rows (roomState.length).
  final int dimX;
...
/// Convert a generated [Level] into the Star Loader int-grid schema.
///
/// The player is relocated within its reachable region if it happens to sit on
/// a target (the game's tile layout has no "player on target" glyph — 'P'
/// would silently drop the target). Moving the player elsewhere in the same
/// region leaves the puzzle and its push-optimal count unchanged.
StarLoaderEntry sokobanToStarLoaderEntry(Level lv) {
  var player = lv.player;
  if (lv.goals.contains(player)) {
    final nbrs = makeNbrs(lv.floors);
    final region = reachable(nbrs, lv.boxes, player);
    int? alt;
    for (final c in region) {
      if (!lv.goals.contains(c) && !lv.boxes.contains(c)) {
        if (alt == null || c < alt) alt = c;
      }
    }
    if (alt != null) player = alt;
  }
  ...
  // one wall cell of padding on every side
  ...
  for (final f in lv.floors) {
    final rr = rowOf(f) - r0, cc = colOf(f) - c0;
    final isGoal = lv.goals.contains(f);
    structure[rr][cc] = isGoal ? slTarget : slFloor;
    if (f == player) {
      state[rr][cc] = slPlayer;
    } else if (lv.boxes.contains(f)) {
      state[rr][cc] = slBox; // structure TARGET => rendered as 'X'
    } else {
      state[rr][cc] = isGoal ? slTarget : slFloor;
    }
  }

  return StarLoaderEntry(
    roomStructure: structure,
    roomState: state,
    optimalMoves: lv.stats['pushes'] as int,
    dimX: rows,
    dimY: cols,
  );
}

/// One-shot: generate a Star Loader level at [difficulty] (1..10).
StarLoaderEntry generateStarLoaderEntry({
  required int difficulty,
  int? seed,
  double? timeBudget,
}) {
  final lv = generate(difficulty, seed: seed, timeBudget: timeBudget);
  return sokobanToStarLoaderEntry(lv);
}
